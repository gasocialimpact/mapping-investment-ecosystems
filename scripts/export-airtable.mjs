...
// IRS 990 / Business Master File extracts built by scripts/fetch-990-data.mjs.
// Joined to organizations by the EIN field in Airtable. Optional: if the files
// are missing every org gets irs: null.
async function loadIrs() {
  ...
  const [f990, bmf] = await Promise.all([load('public/data/irs990-georgia.json'), load('public/data/irs-bmf-georgia.json')]);
  const by990 = new Map((f990?.organizations ?? []).map((o) => [o.ein, o]));
  const byBmf = new Map((bmf?.organizations ?? []).map((o) => [o.ein, o]));
  if (f990 || bmf) console.log(`Loaded IRS data: ${by990.size} 990 filers, ${byBmf.size} BMF organizations`);
  else console.warn('IRS extracts not found — organizations will have irs: null');
  return { by990, byBmf };
}

const LATEST_FILING_KEYS = [
  'taxYear', 'form', 'periodEnd', 'revenue', 'contributions', 'programRevenue', 'investmentIncome',
  'expenses', 'grantsPaid', 'salaries', 'netIncome', 'assetsEOY', 'liabilitiesEOY', 'netAssetsEOY',
  'employees', 'volunteers', 'cashAndSavingsEOY', 'pledgesReceivableEOY', 'notesLoansReceivableEOY',
  'landBuildingsNetEOY', 'investPublicSecuritiesEOY', 'investOtherSecuritiesEOY', 'investProgramRelatedEOY',
  'grantsPayableEOY', 'taxExemptBondsEOY', 'mortgagesNotesPayableEOY', 'unsecuredNotesPayableEOY',
  'netAssetsUnrestrictedEOY', 'netAssetsTempRestrictedEOY', 'netAssetsPermRestrictedEOY',
];

function buildIrs(ein, { by990, byBmf }) {
  if (!ein) return null;
  const n = by990.get(ein) ?? null;
  const b = byBmf.get(ein) ?? null;
  if (!n && !b) return { ein, found: false };
  const filings = n?.filings ?? [];
  const last = filings.length ? filings[filings.length - 1] : null;
  const latest = last ? Object.fromEntries(LATEST_FILING_KEYS.map((k) => [k, last[k] ?? null])) : null;
  return {
    ein,
    found: true,
    legalName: b?.name ?? n?.name ?? null,
    subsection: b?.subsection ?? (n?.is501c3 ? '501(c)(3)' : null),
    ntee: b?.ntee ?? null,
    nteeMajor: b?.nteeMajor ?? null,
    foundation: b?.foundation ?? null,
    rulingYear: b?.rulingYear ?? null,
    yearFormation: n?.yearFormation ?? null,
    mission: n?.mission ?? null,
    bmf: b ? { assets: b.assets ?? null, income: b.income ?? null, revenue: b.revenue ?? null, taxPeriod: b.taxPeriod ?? null } : null,
    latest,
    trend: filings.map((f) => ({
      taxYear: f.taxYear, revenue: f.revenue ?? null, expenses: f.expenses ?? null,
      assetsEOY: f.assetsEOY ?? null, netAssetsEOY: f.netAssetsEOY ?? null,
    })),
  };
}

// National percentiles by county FIPS, from the place dataset generated by
// scripts/fetch-place-data.mjs (authoritative — the CSV-derived ranking above
// only covers whichever counties the CSV parse yields).
async function loadPlacePercentiles() {
  try {
    const raw = await readFile(resolve(ROOT, 'public/data/place-counties.json'), 'utf8');
    const place = JSON.parse(raw);
    const byFips = new Map(place.counties.map((c) => [c.fips, c.pctiles?.[0] ?? null]));
    console.log(`Loaded national percentiles for ${byFips.size} counties from place-counties.json`);
    return byFips;
  } catch {
    console.warn('place-counties.json not found — cviNationalPercentile will be null');
    return null;
  }
}
...
async function main() {
  ...
  const irsData = await loadIrs();

  const organizations = orgRecs.map((r) => {
    const f = r.fields;
    const name = scalar(f['Org. Name']) || 'Unknown';
    const id = recIdToOrgId.get(r.id);
    const einDigits = (scalar(f['EIN']) || '').toString().replace(/\D/g, '');
    const ein = einDigits.length === 9 ? einDigits : null;
    ...
    return {
      ...
      ein,
      einMatch: scalar(f['EIN Match']) || null,
      irs: buildIrs(ein, irsData),
      locationId,
      contactIds: [],
      capitalFlowIds,
  ...
  const placePctiles = await loadPlacePercentiles();
  ...
  const locations = locRecs.map((r) => {
    ...
    if (cviNationalPercentile == null && placePctiles && fipsCode) {
      const p = placePctiles.get(String(fipsCode).padStart(5, '0'));
      if (p != null) cviNationalPercentile = Math.round(p);
    }
  ...
  const withIrs = organizations.filter((o) => o.irs?.found).length;
  console.log(`  ${withIrs} organizations joined to IRS data (${organizations.filter((o) => o.irs?.latest).length} with 990 financials)`);
